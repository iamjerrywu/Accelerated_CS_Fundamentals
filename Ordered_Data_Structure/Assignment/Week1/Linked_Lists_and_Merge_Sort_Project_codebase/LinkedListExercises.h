...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if ((head_ == NULL) || (head_->data >= newNode->data)) {
    // if the list not empty, update prev pointer; otherwise, update the tail pointer
    if (head_ != NULL) {
      head_->prev = newNode;
    } else {
      tail_ = newNode;
    ...
    // if current not the tail_, update prev pointer; otherwise, udpate tail pointer
